...
export class ListaComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    console.log(this.activateRouter.snapshot.params.type);
    ...
    this.activateRouter.params
      .pipe(
        switchMap(
          ({ idArea }) => (
            (this.idArea = idArea),
            this.protocolosSvc.getProtocolos(
              idArea,
              this.user.idEspecialidad,
              environment.EMPTY
            )
          )
        )
      )
      .subscribe((protocolos) => {
  ...
  onOpenModal(protocolo = {}): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      this.protocolosSvc
        .getProtocolos(this.idArea, this.user.idEspecialidad, environment.EMPTY)
        .subscribe((protocolos) => {
          this.dataSource.data = protocolos.response;
        });
    });
